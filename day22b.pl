...
{ package Grid;
  ...
  sub move {
    ...
    my $status = $self->{ grid }{ $pos } || 'c';

    if ($status eq 'c' || $status eq 'i') {
      my $right = $status eq 'i';
      if ($self->{ dir }[0] == 1) {
        $self->{ dir } = $right ? [ 0, 1 ] : [ 0, -1 ];
       }
      elsif ($self->{ dir }[0] == -1) {
        $self->{ dir } = $right ? [ 0, -1 ] : [ 0, 1 ];
       }
      elsif ($self->{ dir }[1] == 1) {
        $self->{ dir } = $right ? [ -1, 0 ] : [ 1, 0 ];
       }
      elsif ($self->{ dir }[1] == -1) {
        $self->{ dir } = $right ? [ 1, 0 ] : [ -1, 0 ];
       }
    ...
    $self->{ grid }{ $pos } = $new_status{ $status };
    $self->{ infected }++ if ($self->{ grid }{ $pos } eq 'i');
